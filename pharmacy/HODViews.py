...
def confirmDelete(request,pk):
    try:
        ...
        messages.error(request, "Patient Cannot be deleted  deleted , Patient is still on medication")
        return redirect('all_patients')
...
def createPharmacyClerk(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, "Failed to Add receptionist!")
            return redirect('add_pharmacyClerk')
...
def deletePharmacyClerk(request,pk):
    try:
        ...
        messages.error(request, "Receptionist Not deleted")
        return redirect('manage_pharmacyClerk')
